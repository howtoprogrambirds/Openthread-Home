...
#include <zephyr.h>
#include "../../coap_server_OT_HOME/interface/coap_server_client_interface.h"
...
static struct k_work multicast_light_work_1;
static struct k_work multicast_light_work_2;
...
static void toggle_mesh_lights_1(struct k_work *item)
{
	static uint8_t command = (uint8_t)THREAD_COAP_UTILS_LIGHT_1_CMD_OFF;

	ARG_UNUSED(item);

	command = ((command == THREAD_COAP_UTILS_LIGHT_1_CMD_OFF) ?
			   THREAD_COAP_UTILS_LIGHT_1_CMD_ON :
			   THREAD_COAP_UTILS_LIGHT_1_CMD_OFF);

	LOG_INF("Send multicast mesh 'light 1' request");
	coap_send_request(COAP_METHOD_PUT,
			  (const struct sockaddr *)&multicast_local_addr,
			  light_option, &command, sizeof(command), NULL);
}

static void toggle_mesh_lights_2(struct k_work *item)
{
	static uint8_t command = (uint8_t)THREAD_COAP_UTILS_LIGHT_2_CMD_OFF;

	ARG_UNUSED(item);

	command = ((command == THREAD_COAP_UTILS_LIGHT_2_CMD_OFF) ?
			   THREAD_COAP_UTILS_LIGHT_2_CMD_ON :
			   THREAD_COAP_UTILS_LIGHT_2_CMD_OFF);

	LOG_INF("Send multicast mesh 'light 2' request");
	coap_send_request(COAP_METHOD_PUT,
			  (const struct sockaddr *)&multicast_local_addr,
			  light_option, &command, sizeof(command), NULL);
}
			    ...
			    mtd_mode_toggle_cb_t on_toggle)
{
	...
	k_work_init(&multicast_light_work_1, toggle_mesh_lights_1);
	k_work_init(&multicast_light_work_2, toggle_mesh_lights_2);
...
void coap_client_toggle_mesh_lights_1(void)
{
	submit_work_if_connected(&multicast_light_work_1);
}

void coap_client_toggle_mesh_lights_2(void)
{
	submit_work_if_connected(&multicast_light_work_2);
}
